...
BOOK_GENERATION_RETRY_ATTEMPTS = 2
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if not _should_retry_visualization_error(exc) or (
                    ...
                    raise
                logger.warning("Visualization analysis failed; retrying", exc_info=True)
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if not _should_retry_visualization_error(exc) or (
                    ...
                    raise
                logger.warning("Visualization code generation failed; retrying", exc_info=True)
                continue

            logger.warning("Visualization code failed local validation; retrying")
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if not _should_retry_visualization_error(exc) or (
                    ...
                    raise
                logger.warning("Visualization repair failed; retrying", exc_info=True)

        raise RuntimeError("Visualization repair retry loop ended unexpectedly")


__all__ = ["BOOK_GENERATION_RETRY_ATTEMPTS", "VisualizePipeline"]
